import { describe, it, expect, beforeEach, jest, afterEach } from '@jest/globals';
import fs from 'fs';

// Store original readFileSync
const originalReadFileSync = fs.readFileSync;

describe('PostQueries', () => {
  let postQueryResolvers: any;
  let mockContext: { requestId: string };

  const mockUsers = [
    { id: '1', name: 'Alice', email: 'alice@example.com', createdAt: '2024-01-01T00:00:00Z' },
    { id: '2', name: 'Bob', email: 'bob@example.com', createdAt: '2024-01-02T00:00:00Z' },
  ];

  const mockPosts = [
    { id: '1', title: 'Post 1', content: 'Content 1', authorId: '1', published: true, createdAt: '2024-01-01T00:00:00Z' },
    { id: '2', title: 'Post 2', content: 'Content 2', authorId: '1', published: false, createdAt: '2024-01-02T00:00:00Z' },
    { id: '3', title: 'Post 3', content: 'Content 3', authorId: '2', published: true, createdAt: '2024-01-03T00:00:00Z' },
  ];

  beforeEach(async () => {
    jest.resetModules();
    mockContext = { requestId: 'test-request-123' };

    // Mock fs.readFileSync to return test data
    (fs.readFileSync as any) = jest.fn((filepath: string) => {
      if (filepath.includes('users.json')) {
        return JSON.stringify(mockUsers);
      }
      if (filepath.includes('posts.json')) {
        return JSON.stringify(mockPosts);
      }
      return originalReadFileSync(filepath, 'utf-8');
    });

    // Dynamic import after mocking
    const module = await import('../../../src/schema/queries/post');
    postQueryResolvers = module.postQueryResolvers;
  });

  afterEach(() => {
    fs.readFileSync = originalReadFileSync;
  });

  describe('Query.posts', () => {
    it('should return all posts', () => {
      const result = postQueryResolvers.Query.posts(null, null, mockContext);
      expect(result).toEqual(mockPosts);
    });

    it('should return array of posts', () => {
      const result = postQueryResolvers.Query.posts(null, null, mockContext);
      expect(Array.isArray(result)).toBe(true);
      expect(result.length).toBe(3);
    });
  });

  describe('Query.post', () => {
    it('should return post by ID', () => {
      const result = postQueryResolvers.Query.post(null, { id: '1' }, mockContext);
      expect(result).toEqual(mockPosts[0]);
    });

    it('should return undefined for non-existent post', () => {
      const result = postQueryResolvers.Query.post(null, { id: '999' }, mockContext);
      expect(result).toBeUndefined();
    });
  });

  describe('Query.publishedPosts', () => {
    it('should return only published posts', () => {
      const result = postQueryResolvers.Query.publishedPosts(null, null, mockContext);
      expect(result.length).toBe(2);
      expect(result.every((post: any) => post.published === true)).toBe(true);
    });
  });

  describe('Post.author', () => {
    it('should return author for a post', () => {
      const post = mockPosts[0];
      const result = postQueryResolvers.Post.author(post, null, mockContext);
      expect(result).toEqual(mockUsers[0]);
    });

    it('should return undefined if author not found', () => {
      const post = { ...mockPosts[0], authorId: '999' };
      const result = postQueryResolvers.Post.author(post, null, mockContext);
      expect(result).toBeUndefined();
    });
  });
});
